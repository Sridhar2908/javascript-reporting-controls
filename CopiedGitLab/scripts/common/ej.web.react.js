(function ($, ej, wd, undefined) {
    ...
    var generateComponent = function (className, widgetName, proto) {
        window[className] = createReactClass({
            observableObj: [],
            ...
            componentWillReceiveProps: function (nextProps) {
                ...
                for (i = 0; i < this.observableObj.length; i++) {
                    ...
                    if (str.indexOf(".") !== -1)
                        str1 = str.split(".");
                    if (str1.length == 2) {
                        if (!ej.isNullOrUndefined(this.props[str1[0]]) && this.props[str1[0]][str1[1]] !== nextProps[str1[0]][str1[1]]) {
                            instance.option(str, nextProps[str1[0]][str1[1]], nextProps[str1[0]][str1[1]] instanceof Array);
                        }
                    }
    ...
    var reportViewerWidget = widgets["boldReportViewer"];
    var reportDesignerWidget = widgets["boldReportDesigner"];
    if (reportViewerWidget) {
        generateComponent('BoldReportViewerComponent', reportViewerWidget.name, reportViewerWidget.proto);
    }
    if (reportDesignerWidget) {
        generateComponent('BoldReportDesignerComponent', reportDesignerWidget.name, reportDesignerWidget.proto);
    }
