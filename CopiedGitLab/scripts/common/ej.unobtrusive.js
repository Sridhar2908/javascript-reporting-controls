...
(function ($, ej, wd, undefined) {
    ...
    var readAttributes = function (element, op, plugin) {
        var attributes = element[0].attributes, att, data = {},
            ...
            dataName = "data-ej-", boldDataName = "data-bold-",
            dataLength = dataName.length, boldDataLength = boldDataName.length,
            ejName = "ej-", boldName = "bold-",
            ejLength = ejName.length, boldLength = boldName.length,
            value;

        for (var i = 0; i < attributes.length; i++) {
            att = attributes[i];
            var isBold = att.name.startsWith(boldDataName) || att.name.startsWith(boldName);
            if (op.dataRole && (att.name.startsWith(dataName) || (att.name.startsWith(boldDataName)))) {
                processAttrib(data, att.name.slice(isBold ? boldDataLength : dataLength), processData(att.value));
            }
            
            if ((op.ejRole && (att.name.startsWith(ejName) || (att.name.startsWith(boldName))) ||
                (op.dataRole === false && op.ejRole === false && op.directive === true && (att.name.startsWith(ejName) || att.name.startsWith(boldName))))) {
                processAttrib(data, att.name.slice(isBold ? boldLength : ejLength), processData(att.value));
            }
        }
