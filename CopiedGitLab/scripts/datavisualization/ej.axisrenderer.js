/* global jQuery, ej, navigator, document */
ej.axisTypes = {};
...
(function ($) {
    ej.EjAxisRenderer.prototype = {
                        ...
                        if (seriesVisibility === 'visible' && pointsLength > 0) {
                            if ((orientation == "horizontal" && !(chartObj.model.requireInvertedAxes)) || (orientation == "vertical" && chartObj.model.requireInvertedAxes)) {
                                ...
                                if (isSpline) {
                                    ...
                                    naturalSpline = params.seriesCollection[seriesKey].naturalSpline = ej.ejSplineSeries.prototype.naturalSpline(visiblePoints, series,yIndex, axis);
                                    if(series.type.toLowerCase() == "splinerangearea")
                                    naturalSpline1 = params.seriesCollection[seriesKey].naturalSpline1 = ej.ejSplineSeries.prototype.naturalSpline(visiblePoints, series, yIndex1, axis);
        ...
        _increaseDateTimeInterval: function (date, interval, intervalType) {
            ...
            } else if (intervalType.toLowerCase() == "weeks") {
                result.setDate(date.getDate() + (interval * 7));
            }
        ...
        _calculateAxisLabels: function (axis) {
            ...
            while (position <= axis.visibleRange.max) {
                if (position >= axis.visibleRange.min && position <= axis.visibleRange.max) {
                    text = (axis._intervalType === 'Weeks' && !(axis.labelFormat)) ? this.getWeekFormat(position) :
                        (ej.format(new Date(position), ((!(axis.labelFormat)) ? ej.EjSvgRender.utils._dateTimeLabelFormat(axis._intervalType, axis) : axis.labelFormat), this.chartObj.model.locale));
        ...
        getWeekFormat: function (position) {
            var day = new Date(position);
            var firstWeek = new Date(day.getFullYear(), 0, 1);
            var week = Math.ceil((((day - firstWeek) / 86400000) + firstWeek.getDay() + 1) / 7);
            return "Week " + week;
        },
        calculateDateTimeNiceInterval: function (axis, startDate, endDate) {
            ...
            interval = this.calculateNumericNiceInterval(totalDays / 7, axis, "weeks");

            if (interval >= 1) {
                return { interval: interval, intervalType: "Weeks" };
            }
    ...
    ej.EjAxisRenderer.prototype = {
            ...
            for (var i = 0; i < labels.length; i++) {
                ...
                if (ej.EjSvgRender.utils._inside(label.Value, range)) {
                                        ...
                                        while (value < pointX + textWidth / 2 && count < 2) {             // intersect
                                            ...
                                            for (var t = 1; t < trimLabelText.length; t++) {
                                                ...
                                                if (!current) {                                                  // to trim next label
                                                    ...
                                                    if (value < pointX + ((i + 1 == labels.length - 1 && xAxis.edgeLabelPlacement == "shift") ? newTextWidth : textWidth / 2)) {
                                                        if (t - 1 != 0)
                                                    ...
                                                    value = nextPoint - nextTextWidth / 2;
                                                    if (value < pointX + ((i == 0 && xAxis.edgeLabelPlacement == "shift") ? newTextWidth: newTextWidth / 2)) {
                                                        if (t - 1 != 0)
                                                            trimLabelText = trimLabelText.toString().substring(0, t - 1) + '... ';
                    ...
                    if (!ej.util.isNullOrUndefined(label.Text)) {
                        ...
                        var isVerticalLabel = (labelRotation == 90 || labelRotation == -90 || labelRotation == 270 || labelRotation == -270);
                        // To set position of edge labels
                        if (xAxis.edgeLabelPlacement && !isVerticalLabel) {
                        ...
                        if (xAxis.labelIntersectAction || xAxis.enableTrim) {
                            ...
                            if (intersectAction == 'none' && !xAxis.enableTrim) {
                                ...
                                if (labelRotation != 0)
                                    textWidth = ej.EjSvgRender.utils.rotatedLabel(xAxis, null, labelRotation, largestText).width;
                                else
                                    ...
                                    for (var k = 0; k < previousLabels.length && !flag; k++) {
                                        ...
                                        for (var l = 0; prevLabel && l < prevLabel.length; l++) {
                                            for (var c = 0; c < collectionLength; c++) {
                                                var temp = 0;
												if(labelRotation != 0)
													textWidth = ej.EjSvgRender.utils.rotatedLabel(xAxis, null,labelRotation,currentLabelColl[c]).width;
												else
                                                    textWidth = ej.EjSvgRender.utils._measureText(currentLabelColl[c], chartAreaWidth, font).width;
                                                var preLabel = prevLabel[l],
                                                    prePoint = Math.abs(Math.floor(ej.EjSvgRender.utils._getPointXY(preLabel.Value, range, xAxis.isInversed) * (xAxis.width)));
											    if(labelRotation != 0)
													textWidth = ej.EjSvgRender.utils.rotatedLabel(xAxis, null,labelRotation,preLabel.Text).width;
												else
                                                    preTextWidth = ej.EjSvgRender.utils._measureText(preLabel.Text, areaBoundWidth, xAxis.font).width;
                                                if (i != xAxisLabelLength)
                                                    textWidth = Math.floor(textWidth / 2);
                                                    if(xAxis.valueType == 'category' && preLabel.Value == 0 && position == "shift" && textWidth >= xAxis.maximumLabelWidth)
                                                       prePoint = xAxis.x;
        ...
        _drawStripline: function (axis, stripLine, axisIndex, stripLineIndex) {
            var valueType = !ej.util.isNullOrUndefined(axis.valueType) ? axis.valueType.toLowerCase() : axis._valueType.toLowerCase();
            ...
            if (stripLine.visible) {
                ...
                if (axis.orientation.toLowerCase() == "horizontal") {
                    ...
                    if ((ej.util.isNullOrUndefined(start)) && (ej.util.isNullOrUndefined(end) || ej.util.isNullOrUndefined(stripWidth))) {
                        isVisible = false;
                    } else {
                        start = (startFromAxis ? ((ej.util.isNullOrUndefined(stripLine.offset)) ? axis.visibleRange.min : axis.visibleRange.min + stripLine.offset) : ((jQuery.type(start) == "date") ? (start).getTime() : start));
                        if (!ej.util.isNullOrUndefined(end) && (jQuery.type(end) == "date")) {
                            end = (end).getTime();
                        }
                        if (isRepeat && !ej.util.isNullOrUndefined(repeatEvery) && (stripWidth || stripBorderWidth)) {
                            var limit = (!ej.util.isNullOrUndefined(repeatUntil) ? (axis._valueType == 'datetime' ? repeatUntil.getTime() : repeatUntil) : (end ? end : axis.visibleRange.max));
                            while (start < limit) {
                    ...
                    if (ej.util.isNullOrUndefined(start) && (ej.util.isNullOrUndefined(end) || ej.util.isNullOrUndefined(stripWidth))) {
                        ...
                        if (isRepeat && !ej.util.isNullOrUndefined(repeatEvery) && (stripWidth || stripBorderWidth)) {
                            var limit = (!ej.util.isNullOrUndefined(repeatUntil) ? repeatUntil : (end ? end : axis.visibleRange.max));
                            while (start < limit) {
        ...
        _getHorizontalValue: function (start, end, axis, stripWidth, stripLine, startFromAxis, valueType) {
            var pointstart = ((startFromAxis ? ((start - axis.visibleRange.min) / (axis.visibleRange.delta)) : ej.EjSvgRender.utils._getPointXY(start, axis.visibleRange, axis.isInversed)) * (axis.width)) + axis.x;
            var pointend = (ej.EjSvgRender.utils._getPointXY(end, axis.visibleRange, axis.isInversed) * (axis.width)) + axis.x;
            var widthMax = (ej.EjSvgRender.utils._getPointXY((valueType === 'datetime' ? this._getDatetimeValue(end, start, stripWidth, axis, (stripWidth ? null : end), stripLine, valueType): (start + (ej.util.isNullOrUndefined(stripLine.width) ? 0 : stripLine.width))), axis.visibleRange, axis.isInversed) * (axis.width)) + axis.x;     
        ...
        _getVerticalValue: function (start, end, axis, stripWidth, stripLine, startFromAxis) {
            var pointYstart = (axis.y + axis.height) - ((startFromAxis ? ((start - axis.visibleRange.min) / (axis.visibleRange.delta)) : ej.EjSvgRender.utils._getPointXY(start, axis.visibleRange, axis.isInversed)) * (axis.height));
            ...
            var widthYValue = (axis.y + axis.height) - (ej.EjSvgRender.utils._getPointXY((start + (ej.util.isNullOrUndefined(stripWidth) ? 0 : stripWidth)), axis.visibleRange, axis.isInversed) * (axis.height));
        ...
        _calculateVerticalStripline: function (yHeight, pointYstart, axis, stripLine) {
            var height = yHeight,
                ...
                textHeight = ej.EjSvgRender.utils._measureText(stripLine.text, width, stripLine.font).height,
                textX, textY;
            if (stripLine.textAlignment == 'middletop') {
                textX = x + width / 2;
                textY = y + textHeight;
